from AbishnoiRobot.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
from AbishnoiRobot import DRAGONS
...
async def Abishnoi_is_admin(chat_id: int):
    status = False
    Abishnoi = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id, filter=ChannelParticipantsAdmins
    ):
        if Abishnoi.id == user.id:
            status = True
            break
    return status
